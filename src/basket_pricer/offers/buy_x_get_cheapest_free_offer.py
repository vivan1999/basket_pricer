...
class BuyXGetCheapestFreeOffer(AbstractBaseOffer):
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.product_skus) != len(set(self.product_skus)):
            raise InvalidOfferConfigError("product sku's list must not contain duplicates")
        if self.quantity<0:
    ...
    def get_eligible_basket_items(self, basket: Basket)-> Optional[tuple[int, List[BasketItem]]]:
    ...
    def calculate_discount(self, basket: Basket) -> Money:
        ...
        for sku, basket_item in basket_items.items():
            if sku in self.product_skus:
                for _ in range(basket_item.qty):
                    all_eligible_items.append((basket_item.product.sku, basket_item.product.price))
        ...
        for group_idx in range(num_groups):
            ...
            total_discount = total_discount + cheapest_item[1] # cheapest_item[1] will give the unit price of the item
    ...
    def __str__(self) -> str:
        ...
        return (
            f"Buy {self.quantity} Get Cheapest Free from sku's : [{products_str}] "
        )
