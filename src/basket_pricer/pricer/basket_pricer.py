...
from src.basket_pricer.pricer.price_summary import PriceSummary,zero_summary,no_discount_summary
...
class BasketPricer:
    def __init__(self, catalogue: Catalogue, offers: Optional[List[AbstractBaseOffer]]):
        if not isinstance(catalogue, Catalogue):
            raise TypeError(f"catalogue must be Catalogue, got {type(catalogue).__name__}")
        self.catalogue = catalogue
        self.offers = offers or []
        for offer in self.offers:
            if not isinstance(offer, AbstractBaseOffer):
                raise TypeError(f"All offers must be AbstractBaseOffer, but got {type(offer).__name__}")
    ...
    def calculate_subtotal_basket(self,basket: Basket) -> Money:
        ...
        for sku, basket_item in basket_items.items():
            if not self.catalogue.has_product(sku):
                logger.error(f"Product '{basket_item.product.name}' from basket is not present in the catalogue")
